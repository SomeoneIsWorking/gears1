...
| **Audio** | `xaudio_null.cpp`, `xma.cpp`, `xma_context.cpp`, `xma_decode.cpp` | **partial** | XMA decodes: replayed against an offline reference at correlation 1.000000 over a full 142 s stream, and the live tap matches too. The title now mixes continuously (81.8% of frames non-silent over a run to gameplay, versus 25% before). **Nothing plays it** — there is still no output device (catalog #39) |
| Input | — | **absent** | |
