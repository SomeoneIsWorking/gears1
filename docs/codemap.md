...
- *Why does nothing render?* → `vd_null_gpu.cpp` executes the command stream but performs no draws in the normal path. `GEARS_DRAW_FRAME=1` runs the whole-frame guest-draw backend (`runtime/gpu_draw.cpp`), which issues **every** draw of a selected frame — including a real post-load scene frame, 176/176 draws, 0 skipped. It is still **black**, but no longer because of textures: 80.7% of texture bindings now carry real guest data (decode verified against real art) and the RT link serves another 13.9%. The residual is that the frame's world geometry (all `quad_list`, draws 32+) contributes **zero** pixels — see `re_frontier.py show draw-backend-frame`. General rendering is the HLE work in `docs/d3d-seam.md`
