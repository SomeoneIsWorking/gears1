...
| **Audio** | `xaudio_null.cpp` | **partial** | The title produces real PCM: the pump drives its callback at 187.5 Hz and it submits frames one-for-one (peak 0.518, max -5.7 dB). Nothing plays them yet, and it falls silent ~19 s in — XMA has no decoder (catalog #39) |
| Input | — | **absent** | |
