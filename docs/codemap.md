...
| **Audio** | `xaudio_null.cpp`, `audio_out.cpp`, `xma.cpp`, `xma_context.cpp`, `xma_decode.cpp` | **partial** | End to end: the pump drives the title's callback, the title mixes (XMA decoding to do it), and SDL plays the result. XMA verified at correlation 1.000000 against an offline reference over a full 142 s stream. Zero device starvation over 10000 frames. Loops and double-buffered streaming are ported but unexercised (catalog #39) |
| Input | — | **absent** | |
...
- *Need an object to outlive a concurrent free with no guest protocol to hook?* → `runtime/reference_barrier.cpp` makes the freeing thread WAIT until no reader holds the block — delay, never skip, because suppressing a free leaks the block and desynchronises the pool's own bookkeeping. Readers that arrive after the wait begins are invisible to it by construction, so a churning reader cannot starve the free into a hang. **It engaged ZERO times against #45**, which is what proved that crash's free happens BETWEEN the guarded calls rather than during one; keep it for the next lifetime question rather than as evidence about that one
