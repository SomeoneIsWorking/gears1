...
| **Audio** | `xaudio_null.cpp`, `audio_out.cpp`, `xma.cpp`, `xma_context.cpp`, `xma_decode.cpp` | **working** | End to end: the pump drives the title's callback, the title mixes (XMA decoding to do it), and SDL plays the result. XMA verified at correlation 1.000000 against an offline reference over a full 142 s stream. Zero device starvation over 10000 frames. Loops and double-buffered streaming are ported but unexercised (catalog #39) |
| Input | — | **absent** | |
