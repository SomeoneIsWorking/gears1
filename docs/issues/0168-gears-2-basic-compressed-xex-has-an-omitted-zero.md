...
The user-owned Gears 2 XEX is basic-compressed and has two valid data/zero blocks. Their expanded size is 0x1228000; the XEX2 security header declares 0x1230000, leaving an exact 0x8000 tail. The checked loader rejects any mismatch before title identity can be computed. The older XenonRecomp loader and xex_patcher expand from the block descriptors and retain the security image size, indicating the missing tail is zero-filled image space rather than a title-specific exception. Fix the shared loader generically and retain refusals for block/output overflow.
