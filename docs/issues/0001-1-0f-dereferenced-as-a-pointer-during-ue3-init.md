...
### Note (2026-07-22)
Traced end to end with core dump + hardware watchpoints on all four physical aliases. The list walker sub_82766F68 reads node->next at +4. Node 0xA06F0354 is otherwise intact (value at +0xC = 0xA06F0308, matching r30 at the fault), so this is one clobbered field, not a bad chain. sub_826ED298 is the list push and does node->next = *head, faithfully copying the head word. Watching the head at 0xA06F032C records exactly two writes, in order, both inside ONE invocation of sub_8273B488: (1) sub_82761CA8 stores 1.0f via 'stfs f0,0(r9)' with 'addi r9,r29,36', i.e. object+0x24 where object r29 = 0xA06F0308; (2) sub_826ED298 stores a node pointer, using as its head the direct return of sub_82761AA0 (r3=r30 at ppc_recomp.112.cpp:15012). So sub_82761AA0 returned 0xA06F032C -- an address 0x24 bytes INSIDE the still-live object at 0xA06F0308. Overlapping hand-out confirmed; open question is which side is wrong (undersized allocation for the 0xA06F0308 object vs premature pool recycle).

### Note (2026-07-22)
ROOT CAUSE FOUND. The size computation sub_826D6610 dispatches through a bctr jump table whose targets are small stubs laid out right after the bctr (r3 += r6*4, +0x14, +4, +0x18, +0xC, +8; base term is (r5+9)*4). XenonRecomp emitted every case as '// ERROR: 0x...' followed by a bare return, because Function::Analyze ends the function at the bctr, so the stubs fell outside [fn.base, fn.base+fn.size). The function therefore returned only the base size term with no per-type adjustment -- objects allocated undersized, so the +0x24 float array of the object at 0xA06F0308 ran past its end and the pool's bump allocator later handed out 0xA06F032C inside it. That is the overlap. Affected 1394 switch cases across 32 files, all silent. Fixed in the fork (extern/XenonRecomp, branch gears): a pass after function collection grows each switch-owning function over its own targets, absorbs the stub 'functions' the linear scan invented, and updates symbols in place; remaining unresolved cases now emit __builtin_debugtrap() instead of a bare return. 1394 -> 60.
