...
The normal draw family is now grounded rather than inferred from call rates:
0x8222CFF8 emits transient auto-index draws, 0x8222D4F8 emits transient
DMA-indexed draws, 0x8222DA48 emits bound-stream auto-index draws, and
0x8222DE50 emits bound-buffer DMA-indexed draws. A title-specific strong wrapper
super-calls each retained recompiled body and compares its semantic arguments
with the packet that body emitted. A headless menu walk through frame 1712
matched all 90,854 observed calls with zero missing or mismatched packets;
0x8222DA48 was not dynamically exercised in that walk. The earlier rate search
compared one logical API call with multiple predicated EDRAM-tile executions of
its packet, so PM4 execution count was not a valid one-to-one oracle. Issue #141
tracks the remaining full-state/resource/resolve semantic observation frontier.
