...
### Note (2026-07-23)
RESOLVED the OPEN item: the vertex fetch constant IS in the fetch register file at the hot-pair draw. The hot VS's vfetch reads vertex fetch constant #95 (Xenia disasm 'vf0' == 95 - storage_index; spirv-dis of vs_5363d074 confirms it reads fetch_constants[0][47][2..3] = fetch-file dwords 190,191 = reg 0x48BE/0x48BF). Captured live (GEARS_DRAW_CAPTURE=1): reg 0x48BE = 0x00097813 0x100000c2 = type 3 (vertex), base 0x97810, size 192 bytes. The earlier 'ZERO type-3 vertex fetch constants' was a scanner artifact: DumpConstantFiles' slot loop only inspects j-entries when the slot's LEADING dword is type 3, and #95 sits at j=2 of slot 31 (leading dword #93 is not type 3), so it was skipped. The catalog #22 instruction patch supplies stride+const-index, not the base.
