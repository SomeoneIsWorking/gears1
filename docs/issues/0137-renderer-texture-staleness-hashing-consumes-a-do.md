...
### Note (2026-08-26)
The validated mechanism was still hidden behind the opt-in `GEARS_DRAW_TEX_DIRTY`, so the default product kept paying the full hashing cost while the codemap described the optimized path as current behavior. Auditing the promotion then found a deeper ordering defect: the tracker cleared soft-dirty bits at the start of a render, erasing every guest write that landed after the previous render but before this one. The movie check had only demonstrated concurrent writes after the clear, not this inter-frame class. The texture owner now consumes the previous observation period first and clears only after its final texture query, and the kernel-backed regression writes between two simulated frames and requires the next frame to report dirty before it may clear. With that correction, the per-frame options owner enables page tracking by default and exposes `GEARS_DRAW_NO_TEX_DIRTY=1` only as the full-hash control. On the current chapter-45 capture, the post-fix interleaved default/control measurement resolved -3.99 ms (33.20 vs 37.19 ms over 94/94 frames, 3.31 ms resolution floor); a default three-frame replay skipped all 244 cached textures and 49.39 MiB with zero short reads, clear failures, or contradictions. Its final image exactly matched the full-hash control (`3b34082ab05198fa4733a50c7fe6e671b32c3871be38f7b563154ec741f80c25`). A rebuilt default-path headless boot armed all four alias windows and detected one of three cached startup textures changing under an unchanged fetch constant, evicting and re-uploading it with zero staleness misses. Evidence: `scratch/logs/texdirty_ordered_default_ab_20260826.log`, `scratch/logs/texdirty_ordered_default_report_20260826.log`, `scratch/logs/texdirty_postfix_{default,control}_hash_20260826.log`, and `scratch/logs/texdirty_ordered_default_live_current_20260826.log`.
