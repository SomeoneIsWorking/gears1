...
| `GEARS_DRAW_REINTERP=1` | **Convert a surface's contents when the frame re-declares its EDRAM base under a different colour format.** EDRAM holds BITS and this renderer's one-wide-image-per-base holds VALUES, which agree only while the format does not change; the pass packs the stored value under the OLD format and unpacks it under the NEW one, so a draw blends against the number the console would have read. Measured on `walk_gameplay.gfr`: 8 conversions, 2 REFUSED (the `k_16_16` pair, whose SNORM scale convention is unverified -- refused and named, never approximated). **OFF by default**: with it on the frame's colour balance moves onto the diagonal (R/G 0.769 -> 0.999, catalog #62's headline) and the picture blows out, which is not yet resolved -- see `catalog.py show 83` |
