...
| `GEARS_DRAW_UNTILE=1` | **Stops emulating the console's predicated EDRAM tiling.** The 360 splits a 1280x720 surface across EDRAM tiles and replays the whole command buffer once per tile with a different scissor band and a `PA_SC_WINDOW_OFFSET` that shifts the world; a host renderer with a full-resolution target has no reason to. Collapses the replays into one pass and merges their resolves. **Refuses unless the replay is provably a replay** — same draw count, same order, same (vertex shader, pixel shader, index count, primitive, colour mask, blend, depth control) on every draw — and reports per-candidate *why* it declined. Measured: 174 of 348 base-pass draws removed on `courtyard`, 185 of 370 on `bright`, 193 of 389 on `play_v2`; output bit-exact on 3 of 4 captures. **It is not a performance win** — see `GEARS_DRAW_AB_UNTILE` and claim C008 |
...
| Knob | Meaning |
|---|---|
| `GEARS_DRAW_DIAG=<path.tsv>` | **The per-draw table.** One row per draw joining what it was, what it did (pipeline statistics) and every piece of state that can zero it, with a `verdict` naming the stage it died at. **Resolves are rows too** (`prim_name=resolve`, with `resolve_dest`/`resolve_src`/`resolve_dst`) — they used to be skipped, which made the frame's UE3 pass boundaries invisible. Start here, then `tools/pass_structure.py` for the pass attribution |
