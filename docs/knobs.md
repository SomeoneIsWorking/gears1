...
| `GEARS_DRAW_PS_CONSTS=<ps hash>` | Print the pixel float constants a named shader actually received, as numbers. `psconst 9/3 nz` says three of nine are non-zero and cannot say WHICH — and for a pass ending in a scale, "the scale is 0.9" and "the scale is 0" are a frame and a black screen. Prints the **raw bits** alongside each float: `-nan` as a word says nothing, while `ffc00000` (an invalid operation) and `ffffffff` (uninitialised memory) point at completely different bugs. It found catalog #73 |
