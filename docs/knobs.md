...
| `GEARS_DRAW_REINTERP_SELFTEST=1` | Run the reinterpretation's compute module on this GPU against five cases with arithmetic answers, before any frame. Two must convert, one must **not change anything**, and the last two are a round trip (7e3 -> 8888 -> 7e3, the second starting from the first's measured output) -- the property that makes converting the whole surface sound rather than destructive. A shader that did nothing passes the negative and fails the rest |
