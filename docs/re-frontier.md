...
- gap: There is no decoder, but the INTERFACE to one is now real and verified: the context array is published in register 0x600, XMACreateContext hands out slots in it, and the title's kick/lock/clear writes land on registers 0x650/0x690/0x6a0 with correct context bits (they previously scattered to 0x7FEA3460 and similar, computed against an array base of zero). What remains is decode itself: an XMA bitstream parser plus a WMA-Pro-class decoder writing blocks to output_buffer_ptr and advancing the offsets the title polls -- and exact kick delivery, which means trapping the register write through PPC_MM_STORE_U32 rather than polling.
