...
- gap: The save is created but its CONTENTS have never been read back and checked, and no load-a-save round trip has run — the checkpoint RESTORE path crashes first (catalog #45). XamUserGetName truncates as the console does, which is why the file is named "Pla" from a 4-byte buffer.
