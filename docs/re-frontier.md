...
- evidence: Chapter 45 adds the outdoor checkpoint. Full authored-mip upload restores identity-matched shadow casters to 31,621 versus oracle 31,618 pixels and 12,147 versus 12,147, with sampled value 0.968719 on both renderers (C063, catalog #109). The fresh post-fix pair has 21/21 structurally shared resolves.
- where: tools/camera_pair.sh; tools/layer_compare.py; tools/draw_interval_ledger.py
- gap: Locate the first exact draw/state divergence in the cached fresh chapter-45 pair. The first 40 draws align exactly after normalizing disabled fragment stages, and their EDRAM ownership matches. In the first D5A0 producer interval, 11 of 13 draws align exactly; the four caster identities are the same but their title-issued order differs, so that is world-state drift rather than renderer behavior. Keep the known different 8,448-index caster, particle/effect-heavy passes, and structurally undecodable rows outside parity denominators. Xenia is verified only for Gears 1; Gears 2 and 3 need independent oracle validation.
...
- gap: Gears 1 guest-address bindings, shader hashes, probes, and scripted policy remain compiled into shared-looking runtime files rather than a factual title adapter. Exact runtime activation is closed, but the ownership separation is not complete.
