...
- gap: The only frame reachable headless is the LOADING phase: 24 point-list draws of 1 vertex plus 2 rectangle-list draws of 3 vertices, all kAutoIndex, covering 0.7% of the frame and shading BLACK over the 1x1 stub texture0 (same texture gap as draw-backend). This is NOT the milestone's 'accumulated scene geometry' bar. Scene geometry is post-load and is blocked by catalog #16: immediately after this first frame the guest calls XamLoaderLaunchTitle with a NULL path (Xenia semantics: exit to dashboard) and the runtime aborts on the unimplemented import, so no post-load frame executes. Reproduced on 8 consecutive headless runs; a single earlier run reached 1380 frames, so the quit is nondeterministic, not headless-specific.
- notes: Milestone steps 1 (generalise beyond the hot pair) and 2 (persistent target, submission order) are IMPLEMENTED and verified on real data. Step 3 (bind a previously-rendered RT as texture0 instead of the stub) is untouched. The backend is frame-agnostic and will render a rich frame unchanged the moment a run reaches one -- GEARS_DRAW_FRAME_AT selects it. The blocking prerequisite is catalog #16, not the draw path.
