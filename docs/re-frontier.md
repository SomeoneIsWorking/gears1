...
- status: in-progress
- deps: title-revision-boundary
- evidence: Scoped alias-aware guest write attribution and raw packet-construction scans grounded four normal Gears 1 draw entry points. runtime/titles/gears1/rhi_bindings.cpp retains and super-calls the draw/texture bodies; the focused shader-setter owner keeps the shader bodies as runtime controls while implementing the exact native state transition. Selective inspection of the locally generated draw bodies recovered both transient allocation return contracts and their four post-call device fields. A headless menu walk through frame 1980 matched 153,214/153,214 draws, including 28,550 transient-vertex and 102,353 transient-indexed guest address/byte ranges; focused controls reject absent resource evidence, wrong addresses, and wrong sizes. The same inspection grounded the vertex- and index-buffer setters. `0x8222AE20` owns 18 vertex streams and writes each object, effective address, remaining byte range, and encoded stride; a frame-780 run matched all 2,463 updates with zero errors across 86,002 bindings. The former vertex-stream label for `0x8222B068` was falsified: the retained body writes one of four colour-target object slots and an interleaved colour descriptor, while `0x8222B398` owns the adjacent depth-target slot and two depth descriptor words. The same run matched 6,481 colour-target and 3,555 depth-target updates; the colour descriptor arm records the body-normalized post-call result and checks the independent device shadow. The bound-index object layout is decoded into a complete allocation, width, and endian view; each draw's checked slice is independently compared with the DMA packet. A prior frame-780 run matched all 3,717 bound-index draws and 3,926 index bindings with zero errors across 24,233 draws and 80,023 bindings. One typed frame event vector preserves draw/binding/present interleaving directly. The transactional shader-setter audit matched 240/240 exact write sets with zero fallbacks and a focused mismatch negative control. Its timing gate rejected default native execution: 1273 ns native versus 1232 ns retained median over 176/176 calls.
- where: runtime/rhi_semantic_stream.*; runtime/gpu_swap_packet.*; runtime/titles/gears1/rhi_bindings.cpp; runtime/titles/gears1/rhi_{index,vertex}_buffer.*; runtime/titles/gears1/shader_setter_{state,override}.*; runtime/guest_write_watch.*; docs/d3d-seam.md; tests/test_rhi_semantic_stream.cpp; tests/test_rhi_index_buffer.cpp; tests/test_rhi_vertex_buffer.cpp; tests/test_gpu_swap_packet.cpp; tests/test_shader_setter_state.cpp
- gap: The bound-vertex draw entry is statically grounded but lacks live coverage. Complete vertex-buffer views, resource creation/lifetime, resolves, retirement, full-stream comparison, and pixel parity do not yet exist, so no native bypass is authorized.
