...
- status: in-progress
- deps: title-revision-boundary
- evidence: Scoped alias-aware guest write attribution and raw packet-construction scans grounded four normal Gears 1 draw entry points. runtime/titles/gears1/rhi_bindings.cpp retains and super-calls those bodies plus exact SetTexture, SetPixelShader, and SetVertexShader bodies, then publishes globally ordered typed observations to runtime/rhi_semantic_stream.* only under GEARS_NATIVE_RHI_OBSERVE=1. A headless menu walk through frame 1712 matched 90,854/90,854 draws. A later headless run through frame 120 matched 734 texture, 118 pixel-shader, and 118 vertex-shader state updates: 970/970 bindings, zero missing or mismatched. Focused tests prove draw and binding mismatch controls plus cross-kind ordering.
- where: runtime/rhi_semantic_stream.*; runtime/titles/gears1/rhi_bindings.cpp; runtime/guest_write_watch.*; docs/d3d-seam.md; tests/test_rhi_semantic_stream.cpp
- gap: The bound-vertex entry is statically grounded but lacks live coverage. Complete state, resource lifetime, resolves, presentation, retirement, full-stream comparison, and pixel parity do not yet exist, so no native bypass is authorized.
