...
- where: runtime/vd_null_gpu.cpp (TrackConstantLoad, DumpConstantFiles); runtime/gpu_diagnostics_profile.*; runtime/titles/gears1/gpu_diagnostics_profile.cpp; GEARS_CONST_DUMP=1
- gap:
...
- where: runtime/vd_null_gpu.cpp CaptureRepresentativeDraw + ReadGuest32Raw; runtime/gpu_diagnostics_profile.*; runtime/titles/gears1/gpu_diagnostics_profile.cpp (GEARS_DRAW_CAPTURE=1, GEARS_DRAW_CAPTURE_ANY drops the representative-shader filter, GEARS_DRAW_CAPTURE_DIR); report scratch/draw-params/hot_draw.txt
...
- status: re-verified
- deps: frame-pair-validated
- evidence: scratch/camerapair_short_ui_20260813 from the shipping camera_pair path with the f450 START/f600 A route after implementing Xenia's XN_SYS_UI open/close protocol. Provenance MATCH with frozen camera digest 5836bee14961d066; both input arms fired; oracle camera f901; native camera matched at 0.61 camera thresholds. Decoded front buffers visibly show the same door and no overlay. A pre-fix frame visibly carries NO STORAGE DEVICE; the post-fix capture shows it gone and CHECKPOINT rendered.
- where: tools/camera_pair.py; tools/resolve_float_stats.py
- gap:
...
- where: tools/camera_pair.py; tools/layer_compare.py; tools/draw_interval_ledger.py
...
- notes: Preserve only the independently useful forwarding semantics as evidence, then delete this static mechanism in `static-product-removal`. The replacement is `gears-dynarec-discriminator`, not another generated forwarder.
...
- notes: Do not run this generator. Remove the generator, switch-target configuration, and this historical step in `static-product-removal` before dynarec implementation.
...
- notes: Exact parsed-image identity must fail closed before any build-specific binding activates. The target binds runtime images through x360port; generated-profile coupling is awaiting replacement.
...
- gap: Move checked XEX/image/import validation into x360port and wire the fresh-clone launcher to load the runtime image without generating guest source. Only the exact Gears 1 identity is supported; additional-title conformance remains outstanding.
...
### static-product-removal — Remove the static generated product first
- status: todo
- deps: clean-distribution-tip, title-revision-boundary
- evidence:
- where: extern/XenonRecomp static translator; generated title module paths; function maps; generator-only configuration/tests/docs
- gap: Preserve independent binary/behavior evidence, title identity, the reusable checked-XEX parsing contract, and native subsystem code; then delete XenonRecomp's translator, generated PPC modules, precomputed function maps, seed/switch generation, generated-symbol tests, static selectors/configuration, and static methodology before dynarec implementation.
- notes: The reusable XEX parser moves to the platform owner rather than being deleted with static translation. The product may fail only at one explicit missing x360port executor boundary. Static execution is never retained as a bridge, compatibility selector, or oracle.

### x360port-xenia-executor — Embed Xenia's existing Xenon dynarecs
- status: todo
- deps: static-product-removal, rom-only-provisioning
- evidence:
- where: target shared/x360port; extern/xenia
...
### x360ue3-platform-engine — Own reusable UE3-on-Xbox-360 contracts
- status: todo
- deps: x360port-xenia-executor, title-revision-boundary
- evidence:
- where: target shared/x360ue3; GearsUE3 title/revision adapters
- gap: Define versioned UE3 ABI descriptions, UE3 RHI semantic operations, title-supplied binding schemas, and engine object/resource/thread/frame lifetime over public x360port interfaces. Keep exact Gears addresses, shader hashes, pass rosters, navigation, save policy, gameplay, conformance, and application composition in GearsUE3.
- notes: The local shared/ue3 checkout is reference material only and is never compiled, linked, packaged, copied, or required by either framework.

### gears-dynarec-discriminator — Execute a real Gears leaf, typed import, and all override paths
- status: todo
- deps: x360port-xenia-executor, title-revision-boundary, rom-only-provisioning
- evidence:
- where: shared/x360port; runtime/titles/gears1
...
- status: todo
- deps: gears-dynarec-discriminator, x360ue3-platform-engine
- evidence:
- where: shared/x360port; shared/x360ue3; runtime; runtime/titles/gears1; headless conformance evidence
...
- gap: The live title remains capped near 30 produced frames/s. The measured compatibility renderer clears 16.67 ms on these workloads but not the native engine's 8.33 ms/120 fps target, and no 60 Hz title simulation enhancement exists.
...
- evidence: With rendering effectively disabled, Gears 1 remains near 30 fps. The host vblank source is already 60 Hz, so renderer removal and vblank frequency do not by themselves remove the title-side frame-production limit. The retained-body timing trace at `0x8221B378`/`0x8221B670`/`0x824A5170` counts the startup Bink path: the `0x82AE8C30` blocked branch accounts for the non-present dispatches, while exact present calls match `VdSwap`. A 45-second headless no-render extension reached about frame 571 with 7,971 render-ring reservations and 571 semantic present boundaries, then about frame 1,142 with 81,643 reservations while the startup producer counters remained at 993 dispatches and 429 blocked calls; semantic presents stayed at 29.9/s. This proves the trace boundary was extended past Bink but does not identify the gameplay limiter.
- where: runtime/frame_production_timing.*; runtime/titles/gears1/frame_production_timing_binding.cpp; title/revision timing bindings; runtime guest timing services; headless timing A/B reports
...
- where: runtime/rhi_semantic_{stream,state}.*; runtime/rhi_{target,texture}_state.h; runtime/rhi_renderer_input.*; runtime/gpu_draw_native_input.*; runtime/native_rhi*; runtime/rhi_resource_reference.*; runtime/rhi_packet_evidence.h; runtime/gpu_swap_packet.*; runtime/guest_write_watch.*; runtime/titles/gears1/rhi_bindings.cpp; runtime/titles/gears1/rhi_texture_descriptor_watch.*; runtime/titles/gears1/rhi_device_state_reset_binding.cpp; runtime/titles/gears1/rhi_resource_{construction,identity,lifetime}*; runtime/titles/gears1/rhi_{index,vertex}_buffer.*; runtime/titles/gears1/rhi_resolve*; runtime/titles/gears1/{color_write_gamma,shader_setter}_{state,override}.*; tests/test_{gpu_draw_native_input,guest_write_watch,rhi_semantic_stream,rhi_semantic_state,native_rhi}*; docs/d3d-seam.md
- gap: The separate bound-vertex draw entry and resource-wrapper constructors are statically grounded but still lack live coverage; release-to-zero destructor effects are also unmodeled. Active six-dword texture fetch descriptors agree for the covered state, but viewport/scissor parity is falsified by the gameplay join: the title-side `0x8222ABF8` shadow reports 720 rows while the guest `PA_SC_WINDOW_SCISSOR_BR` command state reports 512. Identify the title producer and capture the complete transition before treating viewport state as native-ready. Texture content and backing-resource realization, shader modules/constants, remaining vertex/output state, and pixels are not yet compared. The title-neutral registry is not a concrete API allocator, and the isolated Vulkan resource/resolve owners remain disconnected until a native producer owns source images and same-binary state/pixel parity exists. No complete frontend bypass is authorized.
- notes: Logical title draw calls are not one-to-one with compatibility-renderer draw executions because predicated Xenos packets replay per EDRAM tile. Exact guest addresses and device offsets remain in the Gears 1 adapter; the semantic stream is title-neutral. `native_rhi_backend.*` now owns the explicit PM4-independent backend transaction: it validates ordering, dispatches a complete plan to a supplied host backend, and requires cancellation after partial refusal. `native_rhi_resources.*` provides its title-neutral guest-object identity and non-boundary lifetime contract. `native_rhi_vulkan_resources.*` provides a separate host Vulkan allocation/lifetime owner whose descriptors contain no guest-object interpretation; the focused Vulkan resolve operation consumes its host image state as backend-side evidence only. Neither owner performs guest object lookup or interprets PM4, EDRAM, or Xenos microcode. No concrete backend consumes the live plan.
