...
- gap: The separate bound-vertex draw entry is statically grounded but lacks live coverage. Resource creation/lifetime, resolves, retirement, full-stream comparison with renderer inputs, and pixel parity do not yet exist, so no native bypass is authorized.
