...
p95 14-35 ms, which is not yet a smooth 60 fps floor. `./run.sh` serves
`/api/perf` on loopback port 32125 from the player's window (`docs/interactive-debug.md`), so windowed play
