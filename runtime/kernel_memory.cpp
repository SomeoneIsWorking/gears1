// Implementations of the Xbox 360 kernel's virtual-memory imports. A future
// x360port adapter binds these handlers through its typed import contract.
...
void __imp__NtAllocateVirtualMemory(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    if (address == 0)
    {
        ...
        lucent::warn("kernel",
                     "NtAllocateVirtualMemory failed, called from {:#x}:"
                     " base={:#x} size={:#x} type={:#x} protect={:#x}",
                     uint32_t(ctx.lr), requestedBase, size, allocationType, protect);
...
void __imp__NtFreeVirtualMemory(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    {
        lucent::debug("kernel",
                      "NtFreeVirtualMemory({:#x}, type {:#x}) decommit only -- reservation kept",
                      address, freeType);
        if ((freeType & kMemDecommit) == 0)
            lucent::warn("kernel", "NtFreeVirtualMemory({:#x}): unexpected FreeType {:#x}", address,
                         freeType);
    }
    ctx.r3.u64 = gears::kStatusSuccess;
}
