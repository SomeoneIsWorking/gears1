...
void __imp__NtCreateEvent(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    const uint32_t handle =
        gears::Handles().Insert(std::make_shared<gears::KernelObject>(kind, initialState));

    if (handlePtr != 0)
        *reinterpret_cast<uint32_t *>(base + handlePtr) = ByteSwap(handle);

    lucent::debug("kernel", "NtCreateEvent(type={}, initial={}) -> {:#x}", type, initialState,
                  handle);
...
void __imp__NtCreateMutant(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    auto object = std::make_shared<gears::KernelObject>(gears::KernelObject::Kind::Mutant, false);
    if (initialOwner)
...
void __imp__NtWaitForMultipleObjectsEx(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    if (waitType == 0)
    {
        ...
        gears::WaitProbe probe("NtWaitAll");
        for (uint32_t i = 0; i < count; i++)
        {
    ...
    gears::WaitProbe probe("NtWaitAny");
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            if (objects[i]->Wait(0))
            {
                ...
                if (gears::t_inAudioPumpCallback)
                    gears::WaitProbeCount("NtWaitAny.rounds", pollRounds);
                return;
            }
        }
