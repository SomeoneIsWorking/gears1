...
void __imp__XexGetModuleSection(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    *reinterpret_cast<uint32_t *>(base + outLength) = ByteSwap(section->size);
    ...
    lucent::debug("kernel", "XexGetModuleSection({}) -> {:#x}+{:#x}", name, uint32_t(section->base),
                  section->size);
    ctx.r3.u64 = gears::kStatusSuccess;
}
...
void __imp__InterlockedPopEntrySList(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    for (;;)
    {
        ...
        replacement.depthSeq =
            ByteSwap(((depthSeq - 0x10000) & 0xFFFF0000) | ((depthSeq + 1) & 0xFFFF));
...
void __imp__RtlImageXexHeaderField(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        {
        case 0x00:
            ctx.r3.u64 = read32(entry + 4);
            break;
        case 0x01:
            ctx.r3.u64 = entry + 4;
            break;
        default:
            ctx.r3.u64 = headerBase + read32(entry + 4);
            break;
        }
...
[[noreturn]] void GuestBugCheck(PPCContext &ctx, uint8_t *base, bool extended)
{
    ...
    const uint32_t returnAddress = uint32_t(ctx.lr);
    const std::string_view description =
        gears::CurrentGuestBugCheckDescription(code, returnAddress);
    lucent::error("kernel",
                  "the title called KeBugCheck{}(code {:#x}) from {:#x}"
                  " -- {}",
                  extended ? "Ex" : "", code, returnAddress,
                  description.empty() ? "an unrecognised bug-check site" : description);
    ...
    if (extended)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (text.empty())
                ...
                lucent::error("kernel", "  parameter {}: {:#x} -> \"{}\"", i + 1, params[i], text);
        }
    }
    ...
    if (!extended)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (!text.empty())
                lucent::error("kernel", "  residual r{}: {:#x} -> \"{}\"", i + 4, residual[i],
                              text);
        }
    }
