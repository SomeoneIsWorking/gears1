...
void Renderer::EnsurePersistentCapacity(uint32_t requiredWidth, uint32_t requiredHeight)
{
    RenderExtentCapacity capacity{requiredWidth, requiredHeight};
    if (persistent && RenderExtentNeedsGrowth(persistent->width, persistent->height, requiredWidth,
                                              requiredHeight))
    {
        capacity = GrowRenderExtentCapacity(persistent->width, persistent->height, requiredWidth,
                                            requiredHeight);
        lucent::info("draw",
                     "persistent renderer capacity growing: {}x{} -> {}x{};"
        ...
        SharedGpuQueueAccess().WaitDeviceIdle(device);
        ReleasePersistent();
    }
    if (!persistent)
    {
