...
struct TextureUploader
{
    TextureUploader(Renderer &r, RendererPersistent &p, const FrameDrawInputs &inputs)
        : R(r), P(p), in(inputs), guestTextures(p.guestTextures), texCache(p.texCache),
          texContentHash(p.texContentHash), samplerCache(p.samplerCache)
    {
    }
...
struct TextureBinder
{
    TextureBinder(RendererPersistent &p, TextureUploader &tx,
                  ...
                  uint64_t textureBindingsPsHash)
        : P(p), TX(tx), resolveDests(resolveDestinations),
          depthResolveDests(depthResolveDestinations), textureBindingsPsHash(textureBindingsPsHash)
    {
    }
    ...
    const uint64_t textureBindingsPsHash;
