                       ...
                       RenderTargetCache &RT, int32_t &x0, int32_t &y0, int32_t &x1, int32_t &y1)
{
    ...
    {
        const uint64_t addr = uint64_t(vf0 & ~uint32_t(3));
        if (addr + 6 * 4 <= in.guestWindowBytes)
        {
            ...
            const float halfPixel = (R[0x2302] & 1) == 0 /*kD3DZero*/ ? 0.5f : 0.0f;
            int32_t fx[3], fy[3];
            for (int k = 0; k < 3; ++k)
            {
                uint32_t rx, ry;
                        ...
                        std::vector<PreparedDraw> &prepared)
{
    ...
    const uint32_t srcBase = srcSelect < 4 ? (R[kColorInfo[srcSelect & 3]] & 0xFFF) : 0xFFFFFFFFu;
    ...
    if (RT.formatsPerBase.count(srcBase) && route != resolveRouting.end())
    {
        ...
        pd.sampleLayout =
            DeriveDrawSampleLayout(sampleModel ? ((pd.surfaceInfo >> 16) & 3) : 0, RT.W, RT.H);
        ...
        pd.resolveSrcFormat = srcSelect < 4 ? ((R[kColorInfo[srcSelect & 3]] >> 16) & 0xF) : 0u;
        ...
        pd.resolveSrcRect.extent = {uint32_t(std::max(0, x1 - x0)), uint32_t(std::max(0, y1 - y0))};
        ...
        // the colour on its way out of EDRAM, and the red/blue swap.
        {
            ...
            static const std::string forced{lucent::config::text("DRAW_RESOLVE_SCALE")};
            if (!forced.empty())
            ...
            static const bool noSwap = lucent::config::flag("DRAW_RESOLVE_NOSWAP");
            if (noSwap)
    ...
    else
    {
        ...
        const bool isDepthResolve = srcSelect >= 4 && droute != resolveRouting.end();
