...
// emitted the draw.
//
// WHAT A NATIVE PASS IS. A SPIR-V module written against UE3's own sources
// (`$GEARS_UE3_SRC`, see docs/native-renderer.md) rather than derived from Xenos
// microcode, plus the binding layout it expects. Everything else about the draw --
// the geometry, the render target, the blend state, the constants -- is unchanged,
// because those come from the guest and the guest is right about them.
//
...
struct Pass
{
    ...
    // Which UE3 source it was written from, so the next reader can check it
    // against the engine rather than against our own reasoning.
