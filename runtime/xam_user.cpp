...
// no Live, one PC-backed storage device, a controller when a host or scripted
// source exists, and automatic handling for the storage selector. Keep these
// answers coherent: titles cross-check them across otherwise unrelated calls.
...
void __imp__XamContentCreateEx(PPCContext& __restrict ctx, uint8_t* base)
{
    ...
    //
    // The first eight integer arguments occupy registers; the ninth is in the
    // caller's parameter-save area. Reading the cache-size register as the
    // overlapped pointer made this operation complete synchronously with no
    ...
    // The observed asynchronous caller contract proceeds only after IO_PENDING
    // and then consumes the real result from the overlapped block. Returning
    // plain success bypassed that completion path and left the requested save
    // unread.
...
void __imp__XamShowDeviceSelectorUI(PPCContext& __restrict ctx, uint8_t* base)
{
    ...
    // Xenia mirrors the console's system-UI lifetime: open notification,
    // overlapped completion, then a delayed close notification. Gears keeps its
    // own modal up until it receives the close event, so completing only the
    // overlapped leaves "NO STORAGE DEVICE" over live gameplay forever.
