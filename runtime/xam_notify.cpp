...
void BroadcastLocked(uint32_t id, uint32_t param)
{
    ...
    lucent::debug("xam", "notification {:#x} param {:#x}: {} listener(s) accepted",
        id, param, accepted);
}
    ...
    std::chrono::milliseconds delay)
{
    std::lock_guard<std::mutex> guard(g_listenerMutex);
    MaterialiseScheduledLocked();
    const ScheduledNotification scheduled{
        std::chrono::steady_clock::now() + delay, {id, param}};
    const auto position = std::upper_bound(g_scheduled.begin(), g_scheduled.end(),
        scheduled.due, [](const auto& due, const auto& item) { return due < item.due; });
...
void __imp__XamNotifyCreateListener(PPCContext& __restrict ctx, uint8_t*)
{
    ...
    lucent::debug("xam", "XamNotifyCreateListener(mask {:#x}, max version {}) -> {:#x}",
        mask, maxVersion, handle);
    ctx.r3.u64 = handle;
}
