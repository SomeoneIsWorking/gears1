...
void __imp__KeInitializeSemaphore(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    *reinterpret_cast<uint32_t *>(base + address + 16) = ByteSwap(uint32_t(limit));
    ...
    lucent::debug("kernel", "KeInitializeSemaphore({:#x}, count={}, limit={})", address, count,
                  limit);
}
...
void __imp__KeReleaseSemaphore(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    *reinterpret_cast<uint32_t *>(base + ctx.r3.u32 + 4) =
        ByteSwap(uint32_t(previous + int32_t(ctx.r5.u32)));
...
{
    lucent::debug("wait", "[{}] KeSetEvent(object {:#x})", gears::GuestThreadName(), ctx.r3.u32);
