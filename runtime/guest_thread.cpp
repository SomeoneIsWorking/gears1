...
//
// This is not cosmetic. An observed audio rendezvous indexes its participant
// slot by the current CPU. Leaving the field zero makes every participant claim
// the same slot, so the barrier never completes (catalog #40).
...
//
// An observed adaptive lock uses the tick count to decide when spinning should
// yield to its blocking path. Leaving it zero makes that decision permanently
// stale and hangs startup with one thread spinning.
//
// The same lock uses kThreadId to recognize its owning thread, where spinning
// cannot help.
...
uint8_t ProcessorNumberFromMask(uint32_t mask)
{
    ...
    // mask. Round-tripping a written mask through the guest-visible query path
    // establishes that behavior independently of the host implementation.
    // Selecting the lowest bit would be indistinguishable for the one-hot masks
    // normally observed, so keep the full-mask rule explicit here.
