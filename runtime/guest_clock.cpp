...
uint32_t GuestClockVblankSlack() { return g_vblankSlack.load(std::memory_order_relaxed); }
...
void InitGuestClock()
{
    ...
    if (slack < 1)
        lucent::error("time", "GEARS_GUEST_CLOCK_VBLANK_SLACK={} would deadlock:"
            " the guest needs a vblank in flight to produce the present that"
            " authorises the next one. Using 1", slack);
    ...
    if (step)
    {
        ...
        case ClockTrigger::kVblankPaced:
            lucent::info("time", "guest clock trigger is VBLANK-PACED: vblanks"
