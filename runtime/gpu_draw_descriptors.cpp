...
uint64_t DescriptorBuilder::KeyFor(const uint32_t *regs, const FrameDrawItem &d,
                                   uint64_t resolveGeneration, const ShaderXlate &vsX,
                                   const ShaderXlate &psX) const
{
    ...
    mix(uint32_t(d.vsHash));
    mix(uint32_t(d.vsHash >> 32));
    mix(uint32_t(d.psHash));
    mix(uint32_t(d.psHash >> 32));
...
bool DescriptorBuilder::Build(const uint32_t *regs, const FrameDrawItem &d,
                              uint64_t resolveGeneration, const ShaderXlate &vsX,
                              const ShaderXlate &psX, VkDescriptorSetLayout vsTexLayout,
                              VkDescriptorSetLayout psTexLayout, const UniformCache &uc,
                              VkDescriptorSet (&sets)[4])
{
    ...
    auto setBuf = [&](VkDescriptorSet s, uint32_t b, VkDescriptorType t, VkDescriptorBufferInfo *bi)
    {
    ...
    {
        ++hits;
        sets[2] = cached->second[0];
        sets[3] = cached->second[1];
        const auto t0 = Clock::now();
        writes.clear();
        writeUniformSets();
        vkUpdateDescriptorSets(R.device, uint32_t(writes.size()), writes.data(), 0, nullptr);
        msWrite += MsSince(t0);
        return true;
    }
    ...
    auto writeTextures = [&](const ShaderXlate &x, VkDescriptorSet set)
    {
        ...
        for (uint32_t j = 0; j < x.samplers.size(); ++j)
        {
            ...
            VkWriteDescriptorSet ws{VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
            ws.dstSet = set;
            ws.dstBinding = uint32_t(x.textures.size()) + j;
            ws.descriptorCount = 1;
