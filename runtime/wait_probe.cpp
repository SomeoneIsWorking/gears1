...
// StartStallDetector.
ProgressChannel g_progress[] = {{"draw"}, {"audio"}, {"selftest.stopped"}, {"selftest.silent"}};
...
ProgressChannel *FindProgress(const char *channel)
{
    ...
    if (!g_unknownChannelReported.exchange(true))
        lucent::error("stall",
                      "NoteGuestProgress(\"{}\") names no known channel:"
...
struct ThreadState
{
    ...
    // Incremented on each outermost wait. The watchdog samples this once per
    // second, which distinguishes one long wait from a thread repeatedly
    // entering the same wait site without putting a clock read in every guest
    // kernel call.
    std::atomic<uint64_t> waitGeneration{0};
    std::atomic<bool> live{true};
    // Watchdog-thread observations, protected by g_threadsMutex.
    const char *observedSite = nullptr;
    uint64_t observedGeneration = 0;
    uint32_t blockedObservationSeconds = 0;
};
...
void ObserveThreadStates()
{
    std::lock_guard<std::mutex> guard(g_threadsMutex);
    for (const auto &state : g_threads)
    {
        const char *site = state->site.load(std::memory_order_acquire);
        const uint64_t generation = state->waitGeneration.load(std::memory_order_relaxed);
        if (site != nullptr && site == state->observedSite &&
            generation == state->observedGeneration)
        {
            ++state->blockedObservationSeconds;
        }
        else
        {
            state->observedSite = site;
            state->observedGeneration = generation;
            state->blockedObservationSeconds = 0;
        }
    }
}
} // namespace
...
void WaitProbeReport()
{
    ...
    for (auto &site : g_sites)
    {
        ...
        if (site.isCount)
            line +=
                " " + std::to_string(site.nanos) + "x in " + std::to_string(site.count) + " calls";
        else
            line +=
                " " + std::to_string(site.nanos / 1000000) + " ms/" + std::to_string(site.count);
        site.count = 0;
        site.nanos = 0;
    }
    if (g_sitesDropped)
    {
        ...
        line += "AND " + std::to_string(g_sitesDropped) +
                " record(s) from sites that did not fit in the " + std::to_string(kMaxSites) +
...
WaitProbe::WaitProbe(const char *site)
    : site_(t_inAudioPumpCallback ? site : nullptr), began_(site_ ? NowNanos() : 0),
      published_(false)
{
    ...
    if (state && state->site.load(std::memory_order_relaxed) == nullptr)
    {
        ...
        state->waitGeneration.fetch_add(1, std::memory_order_relaxed);
...
void ReportStall(const char *channel, uint32_t seconds, uint64_t kernelCalls)
{
    ...
    for (const auto &state : g_threads)
    {
        ...
        {
            ++blocked;
            lucent::warn("stall", "  {} is in {} (unchanged across {} watchdog observation(s))",
                         state->name, site, state->blockedObservationSeconds);
        }
    }
    lucent::warn("stall", "{} thread(s) blocked, {} running guest code. {}", blocked, running,
...
void StartStallDetector()
{
    ...
    std::call_once(
        ...
        []
        {
            // A watchdog's normal output is nothing, so a broken one and a healthy
            ...
            {
                lucent::info("stall",
                             "self-test: two synthetic channels armed."
            ...
            std::thread(
                ...
                {
                    lucent::info("stall",
                                 "stall detector armed: reports after {} s with no"
                    ...
                    for (;;)
                    {
                        ...
                        for (auto &channel : g_progress)
                        {
                            ...
                            if (now == 0)
                            {
                                ...
                                lucent::warn("stall",
                                             "{} has NEVER made progress in the"
                                             " {} s since the detector armed -- not a stall, a"
                                             " subsystem that never started:",
                                             channel.name, channel.deadSeconds);
                                ReportStall(channel.name, channel.deadSeconds, calls - lastCalls);
                                continue;
                            }
                            if (channel.reported)
                                continue;
