...
{
  public:
    BitReader(const uint8_t *buffer, size_t sizeInBits) : buffer_(buffer), sizeBits_(sizeInBits) {}

    size_t OffsetBits() const { return offsetBits_; }
    size_t BitsRemaining() const { return sizeBits_ - offsetBits_; }

    void SetOffset(size_t offsetBits) { offsetBits_ = std::min(offsetBits, sizeBits_); }
    ...
    {
        const size_t offsetBytes = std::min(offsetBits_ >> 3, (sizeBits_ - 64) >> 3);
    ...
    size_t Copy(uint8_t *dest, size_t bits)
    {
        ...
        if (relOffsetBits)
        {
            ...
            const uint8_t clearMask = uint8_t(~((uint8_t(1) << relOffsetBits) - 1));
        ...
        if (bitsLeft >= 8)
        {
            std::memcpy(dest + outBytes, buffer_ + (offsetBits_ >> 3), bitsLeft / 8);
            outBytes += bitsLeft / 8;
...
uint8_t PacketFrameCount(const uint8_t *packet)
{
    return packet[0] >> 2;
}
uint8_t PacketMetadata(const uint8_t *packet)
{
    return packet[2] & 0x7;
}
bool IsPacketXma2(const uint8_t *packet)
{
    return PacketMetadata(packet) == 1;
}
uint8_t PacketSkipCount(const uint8_t *packet)
{
    return packet[3];
}
uint32_t PacketFrameOffset(const uint8_t *packet)
{
    const uint32_t value =
        uint32_t(((packet[0] & 0x3) << 13) | (packet[1] << 5) | (packet[2] >> 3));
    return value + 32;
}
...
{
  public:
    RingWriter(uint8_t *buffer, uint32_t capacity) : buffer_(buffer), capacity_(capacity) {}
...
void XmaHwContext::TapWrite(const uint8_t *bytes, size_t count)
{
    if (!tapChecked_)
    {
        ...
        if (!dir.empty())
        {
            ...
            const std::string path = dir + "/ctx" + std::to_string(index_) + ".pcm";
            tap_ = std::fopen(path.c_str(), "wb");
            if (!tap_)
                lucent::warn("xma", "context {}: tap {} did not open", index_, path);
        }
    }
    if (tap_)
...
{
    std::lock_guard<std::mutex> guard(mutex_);
    std::memset(memory.Translate<uint8_t>(guestPointer), 0, sizeof(XmaContextData));
}
...
XmaHwContext::PacketInfo XmaHwContext::GetPacketInfo(uint8_t *packet, uint32_t frameOffset)
{
    PacketInfo info;
    ...
    while (true)
    {
        if (stream.BitsRemaining() < kBitsPerFrameHeader)
        {
            ...
            splitHeaderPending = info.frameCount > 0 && stream.BitsRemaining() > 0;
            break;
        }
    ...
    if (IsPacketXma2(packet))
    {
        ...
        {
            lucent::warn("xma",
                         "context {}: packet header says {} frames,"
                         " the chain walk found {}",
                         index_, headerFrameCount, info.frameCount);
        }
    }
    else if (splitHeaderPending)
    {
        ...
        lucent::debug("xma",
                      "context {}: split frame header after frame {}"
                      " (XMA1, detected by continuation bit)",
                      index_, info.frameCount);
        if (info.currentFrameSize == 0)
            info.currentFrame = info.frameCount;
        ++info.frameCount;
    }
...
XmaHwContext::PacketHandle XmaHwContext::GetPacketHandle(XmaContextData *data, uint32_t bufferIndex,
                                                         uint32_t packetIndex,
                                                         uint32_t currentPacketCount)
{
    PacketHandle handle;
    ...
    {
        lucent::error("xma",
                      "context {}: buffer {} marked valid but is a null"
                      " pointer",
                      index_, bufferIndex);
        return handle;
    }
    ...
    {
        lucent::error("xma",
                      "context {}: packet {} is past buffer {}'s {}"
                      " packets",
                      index_, packetIndex, bufferIndex,
...
const uint8_t *XmaHwContext::GetNextPacket(GuestMemory &memory, XmaContextData *data,
                                           uint32_t nextPacketIndex, uint32_t currentPacketCount)
{
    const PacketHandle handle =
        GetPacketHandle(data, data->currentBuffer, nextPacketIndex, currentPacketCount);
    if (!handle.valid)
        return nullptr;
    return memory.Translate<uint8_t>(data->GetInputBufferAddress(handle.bufferIndex)) +
...
uint32_t XmaHwContext::GetNextPacketReadOffset(uint8_t *buffer, uint32_t nextPacketIndex,
...
uint32_t XmaHwContext::GetNextPacketReadOffset(GuestMemory &memory, XmaContextData *data,
                                               ...
                                               uint32_t currentPacketCount)
{
    ...
    return GetNextPacketReadOffset(
        memory.Translate<uint8_t>(data->GetInputBufferAddress(handle.bufferIndex)),
...
void XmaHwContext::UpdateLoopStatus(XmaContextData *data)
{
    ...
    const uint32_t loopStart = std::max(kBitsPerPacketHeader, uint32_t(data->loopStart));
    const uint32_t loopEnd = std::max(kBitsPerPacketHeader, uint32_t(data->loopEnd));

    if (data->inputBufferReadOffset != loopEnd)
        return;

    lucent::debug("xma", "context {}: loop to {} ({} remaining)", index_, loopStart,
                  uint32_t(data->loopCount));
...
bool XmaHwContext::PrepareDecoder(uint32_t sampleRateId, bool stereo)
{
    ...
    if (decoder_.IsOpen() && decoder_.sampleRate() == sampleRate && decoder_.channels() == channels)
        return true;
...
void XmaHwContext::Decode(GuestMemory &memory, XmaContextData *data)
{
    ...
    uint8_t *currentInputBuffer = memory.Translate<uint8_t>(data->GetCurrentInputBufferAddress());
    ...
    {
        lucent::debug("xma",
                      "context {}: read offset {} is inside the packet"
                      " header, clamping to {}",
                      index_, uint32_t(data->inputBufferReadOffset), kBitsPerPacketHeader);
        data->inputBufferReadOffset = kBitsPerPacketHeader;
    }

    const uint32_t currentInputSize = data->GetCurrentInputBufferPacketCount() * kBytesPerPacket;
    const uint32_t currentInputPacketCount = currentInputSize / kBytesPerPacket;

    const int16_t packetIndex = GetPacketNumber(currentInputSize, data->inputBufferReadOffset);
    if (packetIndex < 0)
    {
        lucent::error("xma",
                      "context {}: read offset {} maps to no packet in"
                      " a {}-byte buffer",
                      index_, uint32_t(data->inputBufferReadOffset), currentInputSize);
        return;
    }
    ...
    {
        lucent::debug("xma",
                      "context {}: skipping split-frame tail in packet"
                      " {} ({} -> {})",
                      index_, packetIndex, relativeOffset, packetFirstFrameOffset);
        data->inputBufferReadOffset = packetIndex * kBitsPerPacket + packetFirstFrameOffset;
    ...
    if (skipCount == 0xFF)
    {
        ...
        const uint32_t nextOffset =
            GetNextPacketReadOffset(memory, data, skipToIndex, currentInputPacketCount);
        if (skipToIndex >= currentInputPacketCount || nextOffset == kBitsPerPacketHeader)
            SwapInputBuffer(data);
    ...
    if (packetInfo.currentFrameSize == 0)
    {
        ...
        if (!nextPacket)
        {
            ...
            lucent::debug("xma",
                          "context {}: split frame header at packet {}"
                          " with no next buffer; swapping",
                          index_, packetIndex);
            SwapInputBuffer(data);
            return;
        }
        std::memcpy(inputBuffer_.data(), packet + kBytesPerPacketHeader, kBytesPerPacketData);
        std::memcpy(inputBuffer_.data() + kBytesPerPacketData, nextPacket + kBytesPerPacketHeader,
                    kBytesPerPacketData);

        BitReader combined(inputBuffer_.data(), (kBitsPerPacket - kBitsPerPacketHeader) * 2);
        ...
        {
            lucent::warn("xma",
                         "context {}: split header resolved to the"
    ...
    BitReader stream(currentInputBuffer, size_t(packetIndex + 1) * kBitsPerPacket);
    stream.SetOffset(data->inputBufferReadOffset);

    const uint32_t bitsToCopy =
        std::min(uint32_t(stream.BitsRemaining()), packetInfo.currentFrameSize);
    if (bitsToCopy == 0)
    {
    ...
    if (packetInfo.IsLastFrameInPacket() && stream.BitsRemaining() < packetInfo.currentFrameSize)
    {
    ...
    BitReader frameStream(inputBuffer_.data(), (kBitsPerPacket - kBitsPerPacketHeader) * 2);
    frameStream.SetOffset(relativeOffset - kBitsPerPacketHeader);

    xmaFrame_.fill(0);
    const uint32_t paddingStart =
        uint32_t(frameStream.Copy(xmaFrame_.data() + 1, packetInfo.currentFrameSize));
...

    {
        ...
        lucent::debug("xma",
                      "context {}: frame at {} ({} bits, packet {},"
                      " {}/{})",
                      index_, uint32_t(data->inputBufferReadOffset), packetInfo.currentFrameSize,
                      packetIndex, uint32_t(packetInfo.currentFrame),
                      uint32_t(packetInfo.frameCount));

        rawFrame_.fill(0);
        const int samples = decoder_.Decode(xmaFrame_.data(), packetBytes, floatSamples_);
        if (samples > 0)
        {
            const uint32_t channels = data->isStereo ? 2 : 1;
            if (uint32_t(samples) != kSamplesPerFrame)
                lucent::warn("xma",
                             "context {}: frame decoded to {} samples,"
                             " not {}",
                             index_, samples, kSamplesPerFrame);
            ...
            const size_t count =
                std::min(size_t(samples) * channels, rawFrame_.size() / kBytesPerSample);
            int16_t *out = reinterpret_cast<int16_t *>(rawFrame_.data());
            for (size_t i = 0; i < count; ++i)
            {
                const float scaled = std::clamp(floatSamples_[i], -1.0f, 1.0f) * 32767.0f;
            ...
            if (framesDecoded_ == 1)
                lucent::info("xma",
                             "context {}: first frame decoded ({} Hz,"
                             " {}, {} bits at offset {})",
                             index_, kIdToSampleRate[std::min(uint32_t(data->sampleRate), 3u)],
                             data->isStereo ? "stereo" : "mono", packetInfo.currentFrameSize,
                             uint32_t(data->inputBufferReadOffset));
            ...
            // nothing yet.
            if (isLoopEndFrame)
                loopFrameOutputLimit_ = uint8_t((data->loopSubframeEnd + 1) << data->isStereo);
            else
                loopFrameOutputLimit_ = 0;
            if (loopStartSkipPending_)
            {
                const uint8_t skip = uint8_t(data->loopSubframeSkip << data->isStereo);
    ...
    if (!packetInfo.IsLastFrameInPacket())
    {
        ...
        data->inputBufferReadOffset = packetIndex * kBitsPerPacket + nextFrameOffset;
        return;
    }

    uint32_t nextInputOffset =
        GetNextPacketReadOffset(memory, data, nextPacketIndex, currentInputPacketCount);

    if (nextPacketIndex >= currentInputPacketCount || nextInputOffset == kBitsPerPacketHeader)
        SwapInputBuffer(data);
    ...
    if (nextInputOffset == kBitsPerPacketHeader)
    {
        ...
        {
            nextInputOffset =
                PacketFrameOffset(memory.Translate<uint8_t>(data->GetCurrentInputBufferAddress()));
            if (nextInputOffset > kMaxFrameSizeInBits)
            {
                lucent::debug("xma",
                              "context {}: next buffer's first packet"
                              " has no frames (offset {})",
                              index_, nextInputOffset);
                SwapInputBuffer(data);
                return;
            }
        }
...
void XmaHwContext::Consume(RingWriter *ring, const XmaContextData *data)
{
    ...
    const uint8_t totalSubframes =
        uint8_t((kBytesPerFrameChannel / kOutputBytesPerBlock) << data->isStereo);

    // Loop-end truncation: discard the rest of the frame past the limit.
    if (loopFrameOutputLimit_ > 0)
    {
        const uint8_t consumed = uint8_t(totalSubframes - currentFrameRemainingSubframes_);
    ...
    const uint8_t effectiveCount = uint8_t(std::max(1u, uint32_t(data->subframeDecodeCount)));
    int8_t subframesToWrite = int8_t(std::min(currentFrameRemainingSubframes_, effectiveCount));
    ...
    const uint8_t frameReadOffset = uint8_t(totalSubframes - currentFrameRemainingSubframes_);
    const uint8_t *source = rawFrame_.data() + size_t(kOutputBytesPerBlock) * frameReadOffset;
    ...
    const int8_t headroom = (currentFrameRemainingSubframes_ - subframesToWrite == 0)
                                ? int8_t(data->outputBufferPadding)
                                : int8_t(0);
    remainingSubframeBlocks_ -= subframesToWrite + headroom;
    currentFrameRemainingSubframes_ = uint8_t(currentFrameRemainingSubframes_ - subframesToWrite);
}
...
void XmaHwContext::StoreContextMerged(const XmaContextData &data, const XmaContextData &initial,
                                      uint8_t *guest)
{
...
void XmaHwContext::Work(GuestMemory &memory, uint32_t guestPointer)
{
    ...
    if (!data.outputBufferBlockCount)
    {
        ...
        lucent::warn("xma",
                     "context {}: kicked with a zero-block output"
                     " buffer",
                     index_);
        return;
    }
    ...
    if (data.interruptWhenDone && framesDecoded_ == 0)
        lucent::warn("xma",
                     "context {}: asks for interrupt-when-done, which"
    ...
    remainingSubframeBlocks_ = int32_t(ring.WriteCount() / kOutputBytesPerBlock);
    ...
    const uint32_t effectiveCount = std::max(1u, uint32_t(data.subframeDecodeCount));
    const int32_t minimumBlocks = int32_t(effectiveCount) + int32_t(data.outputBufferPadding);
    ...
    while (remainingSubframeBlocks_ >= minimumBlocks)
    {
        ...
        if (preSubframes == 0 && data.inputBufferReadOffset == preOffset &&
            currentFrameRemainingSubframes_ == 0)
        {
            lucent::debug("xma", "context {}: decode stalled at offset {}", index_, preOffset);
            break;
        }
    }
