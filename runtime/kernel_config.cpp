...
void __imp__ExGetXConfigSetting(PPCContext &__restrict ctx, uint8_t *base)
{
    ...
    if (!value)
    {
        ...
        lucent::error("config",
                      "XConfig category {:#x} setting {:#x}: {} byte"
                      " buffer -> UNKNOWN, refused",
                      category, setting, bufferSize);
    ...
    lucent::info("config",
                 "XConfig category {:#x} setting {:#x}: {} byte buffer"
                 " -> answered with {} bytes",
                 category, setting, bufferSize, size);
