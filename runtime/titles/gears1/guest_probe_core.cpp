...
void DumpNearbyText(uint32_t around)
{
    ...
    for (int32_t offset = -kBefore; offset <= kAfter && found < 8; ++offset)
    {
        ...
        if (run.size() >= 8)
        {
            lucent::error("fatal", "  text near the fatal frame at {:#x}: \"{}\"", runStart, run);
            ++found;
        }
        run.clear();
    }
...
void ReportEntry(const char *entry, const PPCContext &ctx)
{
    ...
    lucent::error("fatal",
                  "the title entered its fatal handler ({}) from {:#x}:"
                  " r3={:#x} r4={:#x} r5={:#x} r6={:#x}, re-entry flag {} -- this entry {}",
                  entry, uint32_t(ctx.lr), ctx.r3.u32, ctx.r4.u32, ctx.r5.u32, ctx.r6.u32, flag,
                  flag == 1 ? "WILL bug-check"
                            : "is the first, and something already went"
                              " wrong to get here");
...
PPC_FUNC(sub_828D0790)
{
    ...
    if (ctx.r3.u32 != 0)
    {
        ...
        const bool plausible = vtable >= PPC_IMAGE_BASE && vtable < PPC_IMAGE_BASE + PPC_IMAGE_SIZE;
        uint32_t slot1 = 0;
        if (plausible)
            slot1 = ByteSwap(*gears::Memory().Translate<uint32_t>(vtable + 4));
        lucent::error("fatal",
                      "  the object it was called on ({:#x}) has vtable"
                      " {:#x} ({}), slot 1 = {:#x}",
                      ctx.r3.u32, vtable,
        ...
        if (ringData != 0 && ringEnd > ringData)
            lucent::error("fatal",
                          "  the render ring spans {:#x}..{:#x}; the"
                          " object at {:#x} is {}",
                          ringData, ringEnd, ctx.r3.u32,
...
PPC_FUNC(sub_821BA838)
{
    ...
    if (seen.fetch_add(1) < 6)
    {
        ...
        lucent::info("probe",
                     "save-blob feeder on object {:#x}: gate [+424]={:#x}"
                     " -> {}; afterwards the array at +420 is {} ({} bytes at {:#x}),"
                     " returned {:#x}",
                     object, gate, gate == 0 ? "RETURNED EARLY, nothing serialised" : "proceeded",
                     data == 0 || count == 0 ? "EMPTY" : "populated", count, data, ctx.r3.u32);
        (void)before;
    }
}
...
PPC_FUNC(sub_8232B548)
{
    ...
    // is progress.
    {
        ...
        const uint32_t count =
            holder ? ByteSwap(*gears::Memory().Translate<uint32_t>(holder + 4)) : 0;
        ...
        if (reported.fetch_add(1) < 4 || populated)
        {
            ...
            const uint32_t carrierCount =
                ByteSwap(*gears::Memory().Translate<uint32_t>(kCarrier + 4));
            lucent::debug("probe",
                          "  the global carrier at {:#x} is {} ({} bytes"
                          " at {:#x}) -- this is what gets copied into the object before"
                          " the read",
                          kCarrier, carrierData == 0 || carrierCount == 0 ? "EMPTY" : "populated",
                          carrierCount, carrierData);
            lucent::debug("probe",
                          "FString deserialise #{}: archive array is {}"
                          " ({} bytes at {:#x}); guest address 4 currently holds {:#x}",
                          reported.load(),
                          buffer == 0 || count == 0 ? "EMPTY -- nothing filled it" : "populated",
                          count, buffer, low4);
        }
    }
    ...
    const uint32_t flagAt16 = ByteSwap(*gears::Memory().Translate<uint32_t>(archive + 16));
    ...
    if (forMapName)
    {
        ...
        if (!dumped.exchange(true))
        {
            ...
            const uint32_t arrayRef = ByteSwap(*gears::Memory().Translate<uint32_t>(archive + 112));
            if (uint64_t(arrayRef) + 8 < PPC_MEMORY_SIZE)
            {
                const uint32_t data = ByteSwap(*gears::Memory().Translate<uint32_t>(arrayRef));
                const uint32_t count = ByteSwap(*gears::Memory().Translate<uint32_t>(arrayRef + 4));
                lucent::debug("linker",
                              "  the archive's byte array is at {:#x}:"
                              " data {:#x}, count {}{}",
                              arrayRef, data, int32_t(count),
                              (data == 0 || count == 0)
            ...
            {
                lucent::debug("linker",
                              "  the archive's byte-array reference"
        ...
        lucent::debug("linker",
                      "map-name FString deserialise #{}: archive {:#x}"
                      " (+16={:#x}, +20={:#x}) -> FString at {:#x} data {:#x} length {}{}",
                      n, archive, flagAt16, t_archiveIsSaving, stringOut, word(stringOut), length,
                      flagAt16 == 0
...
{
    lucent::debug("linker",
                  "map-name FString probe: {} deserialise(s) seen from"
                  ...
                  " deserialise is fine",
                  g_fstringFromMapChange.load(), g_fstringLoads.load());
}
} // namespace gears::titles::gears1
...
PPC_FUNC(sub_82170408)
{
    ...
    {
        lucent::error("probe",
                      "container growth to {} elements x {} bytes ="
                      " {:#x}: the count came from a field holding {:#x} ({} as a signed"
                      " integer, {} as a float)",
                      count, ctx.r4.u32, size, count, int32_t(count),
                      *reinterpret_cast<const float *>(&count));
        ...
        if (t_serialisingArchive != 0)
        {
            ...
            for (uint32_t i = 0; i < 128; i += 4)
                dump.add(" {:08x}",
                         ByteSwap(*gears::Memory().Translate<uint32_t>(t_serialisingArchive + i)));
            ...
            const uint32_t vtable =
                ByteSwap(*gears::Memory().Translate<uint32_t>(t_serialisingArchive));
            lucent::Line slots;
            slots.add("  archive vtable {:#x} slots:", vtable);
            for (uint32_t i = 0; i < 8; ++i)
                slots.add(" {:08x}",
                          ByteSwap(*gears::Memory().Translate<uint32_t>(vtable + i * 4)));
            ...
            const uint32_t sourceObject =
                ByteSwap(*gears::Memory().Translate<uint32_t>(t_serialisingArchive + 112));
            const uint32_t position =
                ByteSwap(*gears::Memory().Translate<uint32_t>(t_serialisingArchive + 108));
            if (sourceObject != 0)
            {
                const uint32_t buffer =
                    ByteSwap(*gears::Memory().Translate<uint32_t>(sourceObject));
                const uint32_t count =
                    ByteSwap(*gears::Memory().Translate<uint32_t>(sourceObject + 4));
                lucent::error("probe",
                              "  reading at offset {} from a {}-byte"
                              " array at {:#x} (holder {:#x})",
                              position, count, buffer, sourceObject);
                if (buffer == 0)
                {
                    ...
                    for (uint32_t i = 0; i < 32; i += 4)
                        low.add(" {:08x}", ByteSwap(*gears::Memory().Translate<uint32_t>(i)));
                ...
                if (buffer != 0)
                {
                    ...
                    for (uint32_t i = 0; i < 48; ++i)
                        bytes.add(" {:02x}", *gears::Memory().Translate<uint8_t>(buffer + i));
        ...
        if (t_serialisingArchive != 0)
            lucent::error("probe",
                          "  inside FString serialisation on a {}"
                          " archive ({:#x}). {}",
...
PPC_FUNC(sub_82214F50)
{
    ...
    if (ctx.r4.u32 >= (64u << 20) && seen.fetch_add(1) < 8)
        lucent::error("probe",
                      "pool allocation of {:#x} bytes ({} MB) requested"
                      " from {:#x}: r3={:#x} r5={:#x} r6={:#x} r7={:#x}",
                      ctx.r4.u32, ctx.r4.u32 >> 20, uint32_t(ctx.lr), ctx.r3.u32, ctx.r5.u32,
                      ctx.r6.u32, ctx.r7.u32);
    __imp__sub_82214F50(ctx, base);
    gears::titles::gears1::NotePoolEvent(ctx.r3.u32, false);
}
...
PPC_FUNC(sub_822151F8)
{
    ...
    {
        lucent::error("probe",
                      "engine realloc of {:#x} bytes ({} MB) requested"
                      " from {:#x}: allocator={:#x} existing pointer={:#x} r6={:#x}"
                      " r7={:#x}",
                      ctx.r5.u32, ctx.r5.u32 >> 20, uint32_t(ctx.lr), ctx.r3.u32, ctx.r4.u32,
                      ctx.r6.u32, ctx.r7.u32);
