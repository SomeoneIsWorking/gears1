...
void LogFormatted(lucent::Level level, const char *format, va_list arguments)
{
    va_list measured_arguments;
    va_copy(measured_arguments, arguments);
    const int measured = std::vsnprintf(nullptr, 0, format, measured_arguments);
    va_end(measured_arguments);
    if (measured < 0)
    {
        lucent::error("system-constants", "diagnostic formatting failed");
        return;
    }

    std::vector<char> buffer(static_cast<size_t>(measured) + 1);
    std::vsnprintf(buffer.data(), buffer.size(), format, arguments);
    std::string message(buffer.data(), static_cast<size_t>(measured));
    while (!message.empty() && (message.back() == '\n' || message.back() == '\r'))
    {
        message.pop_back();
    }
    lucent::log(level, "system-constants", message);
}
...
void Report(const char *format, ...)
{
    ...
    LogFormatted(lucent::Level::Info, format, arguments);
    va_end(arguments);
}
...
void ReportError(const char *format, ...)
{
    va_list arguments;
    va_start(arguments, format);
    LogFormatted(lucent::Level::Error, format, arguments);
    va_end(arguments);
}
...
void ReportWarning(const char *format, ...)
{
    va_list arguments;
    va_start(arguments, format);
    LogFormatted(lucent::Level::Warn, format, arguments);
    va_end(arguments);
}
...
int main(int argc, char **argv)
{
    ...
    in = !pos.empty() ? pos[0] : std::filesystem::path("scratch/bin/regfile_hotpair.bin");
    ...
    {
        ReportError("%s: %zu bytes, need at least %zu (0x%zx dwords)\n", in.string().c_str(),
                    raw.size(), RegisterFile::kRegisterCount * sizeof(uint32_t),
    ...
    // exactly right.
    RegisterFile regs;
    std::memcpy(regs.values, raw.data(), RegisterFile::kRegisterCount * sizeof(uint32_t));

    if (have_initiator_override)
    {
        regs.values[XE_GPU_REG_VGT_DRAW_INITIATOR] = initiator_override;
        Report("[note] VGT_DRAW_INITIATOR overridden to %#010x "
    ...
    if (vgt_draw_initiator.value == 0)
        stubs.draw_initiator_stale = true;
    ...
    for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i)
    {
            ...
            color_info.color_format == xenos::ColorRenderTargetFormat::k_16_16_16_16)
        {
            ...
            ReportWarning("RT%u is a 16_16 format; color_exp_bias may need "
    ...
    Report("system-constants UBO for %s\n", in.string().c_str());
    Report("  sizeof(SystemConstants) = %zu bytes  -> %s\n", sizeof(sc), out.string().c_str());
    Report("\n--- register inputs ---\n");
    Report("  PA_CL_VTE_CNTL   = %#010x  (xy_fmt=%u z_fmt=%u w0_fmt=%u, "
           "vport ena x[s%u o%u] y[s%u o%u] z[s%u o%u])\n",
           pa_cl_vte_cntl.value, pa_cl_vte_cntl.vtx_xy_fmt, pa_cl_vte_cntl.vtx_z_fmt,
           pa_cl_vte_cntl.vtx_w0_fmt, pa_cl_vte_cntl.vport_x_scale_ena,
           pa_cl_vte_cntl.vport_x_offset_ena, pa_cl_vte_cntl.vport_y_scale_ena,
           pa_cl_vte_cntl.vport_y_offset_ena, pa_cl_vte_cntl.vport_z_scale_ena,
           pa_cl_vte_cntl.vport_z_offset_ena);
    Report("  PA_CL_CLIP_CNTL  = %#010x  (clip_disable=%u dx_clip_space_def=%u)\n", pcl.value,
           pcl.clip_disable, pcl.dx_clip_space_def);
    Report("  VPORT scale/off  = X(%.1f,%.1f) Y(%.1f,%.1f) Z(%.1f,%.1f)\n",
    ...
    Report("  RB_SURFACE_INFO  = %#010x  (surface_pitch=%u msaa=%u)\n", rb_surface_info.value,
           rb_surface_info.surface_pitch, uint32_t(rb_surface_info.msaa_samples));
    Report("  VGT_DRAW_INITIATOR=%#010x (prim_type=%u)\n", vgt_draw_initiator.value,
           uint32_t(vgt_draw_initiator.prim_type));
    Report("  RB_COLORCONTROL  = %#010x  (alpha_test_en=%u func=%u a2m_en=%u)\n",
           rb_colorcontrol.value, rb_colorcontrol.alpha_test_enable,
           uint32_t(rb_colorcontrol.alpha_func), rb_colorcontrol.alpha_to_mask_enable);
    Report("  RB_ALPHA_REF     = %g\n", rb_alpha_ref);
    Report("  VGT_INDX_OFFSET  = %d\n", vgt_indx_offset);

    Report("\n--- viewport (Xenia draw_util::GetHostViewportInfo) ---\n");
    Report("  xy_offset=(%u,%u) xy_extent=(%u,%u) z=[%.4f,%.4f]\n", viewport_info.xy_offset[0],
           viewport_info.xy_offset[1], viewport_info.xy_extent[0], viewport_info.xy_extent[1],
           viewport_info.z_min, viewport_info.z_max);
    Report("  ndc_scale =(%g, %g, %g)\n", viewport_info.ndc_scale[0], viewport_info.ndc_scale[1],
           viewport_info.ndc_scale[2]);
    Report("  ndc_offset=(%g, %g, %g)\n", viewport_info.ndc_offset[0], viewport_info.ndc_offset[1],
    ...
    Report("  [+%3zu] flags                    = %#010x\n", off(&sc.flags), sc.flags);
    Report("  [+%3zu] vertex_index_load_address= %#010x   (STUB: draw-params)\n",
           off(&sc.vertex_index_load_address), sc.vertex_index_load_address);
    Report("  [+%3zu] vertex_index_endian      = %u          (STUB: draw-params)\n",
           off(&sc.vertex_index_endian), uint32_t(sc.vertex_index_endian));
    Report("  [+%3zu] vertex_base_index        = %d\n", off(&sc.vertex_base_index),
           sc.vertex_base_index);
    Report("  [+%3zu] ndc_scale                = (%g, %g, %g)\n", off(sc.ndc_scale),
           sc.ndc_scale[0], sc.ndc_scale[1], sc.ndc_scale[2]);
    Report("  [+%3zu] ndc_offset               = (%g, %g, %g)\n", off(sc.ndc_offset),
           sc.ndc_offset[0], sc.ndc_offset[1], sc.ndc_offset[2]);
    Report("  [+%3zu] alpha_test_reference     = %g\n", off(&sc.alpha_test_reference),
           sc.alpha_test_reference);
    Report("  [+%3zu] alpha_to_mask            = %#010x\n", off(&sc.alpha_to_mask),
           sc.alpha_to_mask);
    Report("  [+%3zu] zpd_fsi_counter_index    = %#010x\n", off(&sc.zpd_fsi_counter_index),
           sc.zpd_fsi_counter_index);
    Report("  [+%3zu] color_exp_bias           = (%g, %g, %g, %g)\n", off(sc.color_exp_bias),
           sc.color_exp_bias[0], sc.color_exp_bias[1], sc.color_exp_bias[2], sc.color_exp_bias[3]);
    Report("  [+%3zu] texture_swizzled_signs   = (STUB: texture-cache, zero)\n",
           off(sc.texture_swizzled_signs));
    Report("  [+%3zu] texture_swizzles         = (STUB: texture-cache/host, zero)\n",
           off(sc.texture_swizzles));
    Report("  [+%3zu] textures_resolved        = %#010x   (STUB: texture-cache)\n",
           off(&sc.textures_resolved), sc.textures_resolved);
    Report("  [+%3zu] texture_integer_scale_bits (STUB: texture-cache, zero)\n",
           off(sc.texture_integer_scale_bits));
    Report("  [+%3zu] edram_* / stencil / blend  (non-FSI path: all zero)\n",
           off(&sc.edram_32bpp_tile_pitch_dwords_scaled));

    Report("\n--- stubbed / underdetermined ---\n");
    if (stubs.index_load_flags)
        Report("  * index-load flags + vertex_index_load_address + vertex_index_endian:\n"
    ...
    if (stubs.texture_signs || stubs.texture_swizzles || stubs.texture_integer_scale ||
        ...
        Report("  * texture_swizzled_signs / texture_swizzles / textures_resolved /\n"
    ...
    if (stubs.draw_initiator_stale)
        Report("  * VGT_DRAW_INITIATOR reads 0 (not yet mirrored from DRAW_INDX by our\n"
