...
echo "# vulkan validation, ${#captures[@]} capture(s); zero VUIDs allowed" | emit
unexpected=0
...
if (( unexpected )); then
    echo "# FAIL: at least one capture raised a VUID" | emit
    exit 1
fi
echo "# PASS: every capture was Vulkan-validation clean" | emit
